...
export class AuthService {
  ...
  async login(input: LoginInput) {
    ...
    if (user.role === "DRIVER") {
      console.log(
        "[AUTH-SERVICE] Checking Driver document for user:",
        user._id,
      );
      ...
      if (!driverProfile) {
        console.log(
          "[AUTH-SERVICE] CRITICAL: Driver document missing for user:",
          user._id,
        ...
        console.log(
          "[AUTH-SERVICE] Fallback Driver profile created for user:",
          user._id,
        );
      } else {
    ...
    if (user.role === "GUIDE" || user.role === "DRIVER") {
      ...
      if (!user.isProfileComplete) {
        console.log(
          "[AUTH-SERVICE] Profile incomplete for user:",
          user._id,
          "profileStep:",
          user.profileStep,
        );
      ...
      if (!user.status || user.status !== "ACTIVE") {
        console.log(
          "[AUTH-SERVICE] Account inactive for user:",
          user._id,
          "status:",
          user.status,
        );
  ...
  async signup(input: SignupInput) {
    ...
    if (input.role === "DRIVER") {
      ...
      try {
        ...
        console.log(
          "[AUTH-SERVICE] Driver profile created successfully:",
          driverProfile._id,
        );
      } catch (driverError) {
        console.error(
  ...
  async googleLogin(idToken: string) {
    ...
    console.log("[AUTH-SERVICE] Google token verified:", {
      email,
      googleId,
      name,
    });
    ...
    if (user.role === "DRIVER") {
      console.log(
        "[AUTH-SERVICE] Checking Driver document for Google login user:",
        user._id,
      );
      const driverProfile = await Driver.findOne({ userId: user._id });
      ...
      if (!driverProfile) {
        console.log(
          "[AUTH-SERVICE] CRITICAL: Driver document missing for Google user:",
          user._id,
          "Creating fallback profile",
        );
        await Driver.create({
        ...
        console.log(
          "[AUTH-SERVICE] Fallback Driver profile created for Google user:",
          user._id,
        );
      } else {
        console.log(
          "[AUTH-SERVICE] Driver profile exists for Google user:",
          driverProfile._id,
        );
      }
    }
    ...
    if (user.role === "GUIDE" || user.role === "DRIVER") {
      if (!user.isEmailVerified) {
        console.log(
          "[AUTH-SERVICE] Email not verified for Google user:",
          user._id,
        );
        throw new BadRequest("EMAIL_NOT_VERIFIED", { role: user.role });
      }
      ...
      if (!user.status || user.status !== "ACTIVE") {
        console.log(
          "[AUTH-SERVICE] Account inactive for Google user:",
          user._id,
          "status:",
          user.status,
        );
  ...
  async googleSignup(input: GoogleSignupInput) {
    ...
    console.log("[AUTH-SERVICE] Google token verified for signup:", {
      email,
      googleId,
      name,
      role,
    ...
    if (user) {
      ...
      if (user.role !== role) {
        console.log("[AUTH-SERVICE] Role mismatch for existing user:", {
          existing: user.role,
          requested: role,
        });
      ...
      await user.save();
      console.log(
        "[AUTH-SERVICE] Existing user updated for Google signup:",
        user._id,
      );
    } else {
      ...
      if (role === "GUIDE") {
        console.log(
          "[AUTH-SERVICE] Creating Guide profile for Google signup user:",
          user._id,
        );
        await Guide.create({
          userId: user._id,
        ...
        console.log(
          "[AUTH-SERVICE] Guide profile created for Google signup user:",
          user._id,
        );
      }
      ...
      if (role === "DRIVER") {
        console.log(
          "[AUTH-SERVICE] Creating Driver profile for Google signup user:",
          user._id,
        );
        try {
          const driverProfile = await Driver.create({
          ...
          console.log(
            "[AUTH-SERVICE] Driver profile created for Google signup user:",
            driverProfile._id,
          );
        } catch (driverError) {
          console.error(
            "[AUTH-SERVICE] Failed to create Driver profile for Google signup:",
            driverError,
          );
          const errorMessage =
            driverError instanceof Error
  ...
  async createPassword(
    ...
    if (user.hasPassword) {
      throw new BadRequest("User already has a password set");
    }
